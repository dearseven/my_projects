 ...
 * controller.addListener(() {
    ...
    刷新
    在无状态组件中用AnimatedBuilder构建按动画widget
    ，，注意这个动画是要点击屏幕才会执行哈
 */
class AnimationWidgetStaggered extends StatefulWidget {
  AnimationWidgetStaggered({Key key}) : super(key: key);

  _AnimationWidgetStaggeredState createState() =>
      _AnimationWidgetStaggeredState();
}

class _AnimationWidgetStaggeredState extends State<AnimationWidgetStaggered>
      ...
      body: GestureDetector(
        ...
        child: Center(
          ...
          child: Container(
            ...
            child: StaggeredAnim(controller: c),
          ),
        ),
      ),
    );
...
/**
 * 这个才是被通过AnimatedBuilder构建的动画组件
 * 通过AnimatedBuilder来构建动画组件
