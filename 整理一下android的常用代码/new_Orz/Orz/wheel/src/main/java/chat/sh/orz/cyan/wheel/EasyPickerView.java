...
public class EasyPickerView extends View {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isSliding || Math.abs(offsetY) > scaledTouchSlop) {
                    isSliding = true;
                    reDraw();
                    //Log.d(TAG, "downY is: " + downY + ",offsetY is" +
                      //      ":" + offsetY);
                    //scroller.startScroll(0, (int) downY,0 , (int) offsetY,200);
                   // invalidate();
                }
                break;
    ...
    protected void onDraw(Canvas canvas) {
        if (null != dataList && dataList.size() > 0) {
            //剪裁出一个窗口 其实内容是显示在窗口里的,
            //联想一下 房间有一窗户,外面的人看到的只能是窗户大小显示的区域
            canvas.clipRect(
                    cx - contentWidth / 2,
                    cy - contentHeight / 2,
                    cx + contentWidth / 2,
            ...
            for (int i = -half; i <= half; i++) {
                ...
                if (isRecycleMode) {
                    ...
                    //Log.d(TAG, "isRecycleMode index=" + index);
                } else {
                    // Log.d(TAG, "is NOT RecycleMode index=" + index);
                }
    ...
    private void finishScroll() {
        ...
        // 更新
        if (null != onScrollChangedListener && isfinished == false)
