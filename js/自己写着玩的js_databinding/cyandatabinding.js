...
(function() {
	...
	/**
	 * @param {Object} watcher function(varObj)
	 */
	...
	/**
	 * 绑定一组radiobox
	 * @param {Object} groupId，就是每个radio的name啊，html是要求一组radio有同样的name
	 * @param {Object} ids
	 * @param {Object} checkstatus
	 * @param {Object} checkedClass
	 * @param {Object} uncheckedClass
	 */
	DataBindding.bindRadioBox = function(groupId, ids, checkstatus, checkedClass, uncheckedClass) {
			...
			el.onclick = (function() {
				...
				for (var i = 0; i < DataBindding.elemts[("#" + gid)].elemts.length; i++) {
					if (DataBindding.elemts[("#" + gid)].elemts[i].checked) { //true|false
						...
						DataBindding.elemts[("#" + gid)].elemts[i].className = DataBindding.elemts[("#" + gid)].elemts[i].className.replace(
							uncheckedClass, "");
						DataBindding.elemts[("#" + gid)].elemts[i].className = DataBindding.elemts[("#" + gid)].elemts[i].className.replace(
							checkedClass, "");
						DataBindding.elemts[("#" + gid)].elemts[i].className += " "+checkedClass;
					} else {
						DataBindding.elemts[("#" + gid)].elemts[i].className = DataBindding.elemts[("#" + gid)].elemts[i].className.replace(
							uncheckedClass, "");
						DataBindding.elemts[("#" + gid)].elemts[i].className = DataBindding.elemts[("#" + gid)].elemts[i].className.replace(
							checkedClass, "");
						DataBindding.elemts[("#" + gid)].elemts[i].className += " "+uncheckedClass;
					}
				}
				DataBindding.elemts[("#" + groupId)].value = DataBindding.elemts[("#" + groupId)].value.substring(0,
					DataBindding.elemts[("#" + groupId)].value.length - 1)
				console.log("2 radio value:" + DataBindding.elemts[("#" + groupId)].value)
		...
		DataBindding.elemts[("#" + groupId)].value = DataBindding.elemts[("#" + groupId)].value.substring(0,
			DataBindding.elemts[("#" + groupId)].value.length - 1)
		console.log("1 radio value:" + DataBindding.elemts[("#" + groupId)].value)
		__onValueChange(groupId)
		/**通过代码设置checkbox的选中状态
		 * @param {Object} checkedStates
		 * [true,false,true],这一句有几个就设置几个
		 */
		DataBindding.elemts[("#" + groupId)].setCheckStatus = function(checkedStates) {
			DataBindding.elemts[("#" + groupId)].value = "";
			for (var i = 0; i < DataBindding.elemts[("#" + groupId)].elemts.length; i++) {
				...
				if (checkedStates[i]) { //true|false
					...
					DataBindding.elemts[("#" + groupId)].elemts[i].className += " "+checkedClass;
				} else {
					DataBindding.elemts[("#" + groupId)].elemts[i].className = DataBindding.elemts[("#" + groupId)].elemts[i].className
						.replace( uncheckedClass, "");
					DataBindding.elemts[("#" + groupId)].elemts[i].className = DataBindding.elemts[("#" + groupId)].elemts[i].className
						.replace(checkedClass, "");
					DataBindding.elemts[("#" + groupId)].elemts[i].className += " "+uncheckedClass;
				}
			}
			DataBindding.elemts[("#" + groupId)].value = DataBindding.elemts[("#" + groupId)].value.substring(0,
				DataBindding.elemts[("#" + groupId)].value.length - 1)
			console.log("3 radio value:" + DataBindding.elemts[("#" + groupId)].value)
			__onValueChange(groupId)
		}
	...
	/**
	 * 绑定一组checkbox
	 * @param {Object} groupId 这一组的id
	 * @param {Object} ids ["id1","id2"]
	 * @param {Object} checkstatus [true,false] 选中状态
	 * @param {Object} checkedClass "input_checked"
	 * @param {Object} uncheckedClass ""input_unchecked""
	 */
	DataBindding.bindCheckBox = function(groupId, ids, checkstatus, checkedClass, uncheckedClass) {
		DataBindding.elemts[("#" + groupId)] = {
			id:groupId,
			value: "",
			type:"checkbox"//多选
		...
		for (var i = 0; i < ids.length; i++) {
			...
			el.onclick = (function() {
				...
				DataBindding.elemts[("#" + groupId)].value = DataBindding.elemts[("#" + groupId)].value.substring(0,
					DataBindding.elemts[("#" + groupId)].value.length - 1)
				console.log("2 group value:" + DataBindding.elemts[("#" + groupId)].value)
				__onValueChange(groupId)
			})
			//attributes["_dbin_for_items"].value;
		}
		DataBindding.elemts[("#" + groupId)].value = DataBindding.elemts[("#" + groupId)].value.substring(0,
			DataBindding.elemts[("#" + groupId)].value.length - 1)
		console.log("1 group value:" + DataBindding.elemts[("#" + groupId)].value)
		__onValueChange(groupId)
		...
		DataBindding.elemts[("#" + groupId)].setCheckStatus = function(checkedStates) {
			...
			DataBindding.elemts[("#" + groupId)].value = DataBindding.elemts[("#" + groupId)].value.substring(0,
				DataBindding.elemts[("#" + groupId)].value.length - 1)
			console.log("3 group value:" + DataBindding.elemts[("#" + groupId)].value)
			__onValueChange(groupId)
		}
		return DataBindding.elemts[("#" + groupId)];
	}
	...
	DataBindding.bindElemt = function(id, initValue) {
		...
		if (type == "input" || type == "INPUT") {
			...
			if (initValue.index != undefined) {
				...
				DataBindding.elemts[("#" + id)].value = DataBindding.elemts[("#" + id)].elemt.options[DataBindding.elemts[("#" +
					id)].index].value;
				DataBindding.elemts[("#" + id)].optValue = DataBindding.elemts[("#" + id)].elemt.options[DataBindding.elemts[("#" +
					id)].index].innerHTML;
	...
	DataBindding.arrOk = function() {
		...
		for (var i = 0; i < allFors.length; i++) {
			...
			if (DataBindding.arrs["#" + objId] != undefined && DataBindding.arrs["#" + objId].rawHtml == undefined) {
			...
			if (DataBindding.arrs["#" + objId].elemt.length > 0) {
				for (var j = 0; j < DataBindding.arrs["#" + objId].elemt.length; j++) {
					...
					for (var k = 0; k < arrs.length; k++) {
						var prop = arrs[k].substring(0, arrs[k].lastIndexOf("}}"))
						if (prop != "") {
							prop = (prop.substring(prop.indexOf(".") + 1))
							//console.log("item." + prop)
							//rawInnerHtml = rawInnerHtml.replace("{_dbin_for{$" + objId + "." + prop + "}}", eval("(" + "item." + prop + ")"));
							...
							rawInnerHtml = rawInnerHtml.replace("{_dbin_for{$" + objId + "." + prop + "}}", v);

						}
					}
					//console.log(arrs);
					// for (var p in item) {
					//     rawInnerHtml = rawInnerHtml.replace("{_dbin_for{$" + objId + "." + p + "}}", 
					//     eval("item."+ p))
					//     count++;
					// }
	...
	DataBindding.setValue = function(obj, value) {
		if (obj.elemt.tagName == "INPUT" || obj.elemt.tagName == "input") {
		...
		} else if (obj.elemt.tagName == "select" || obj.elemt.tagName == "SELECT") {
			...
			if (value.index != undefined) {
				...
				DataBindding.elemts[("#" + obj.elemt.id)].value = DataBindding.elemts[("#" + obj.elemt.id)].elemt.options[
					DataBindding.elemts[("#" + obj.elemt.id)].index].value;
				DataBindding.elemts[("#" + obj.elemt.id)].optValue = DataBindding.elemts[("#" + obj.elemt.id)].elemt.options[
					DataBindding.elemts[("#" + obj.elemt.id)].index].innerHTML;
				//console.log("####:"+ DataBindding.elemts[("#" + id)].value)
				...
				for (var i = 0; i < DataBindding.elemts[("#" + obj.elemt.id)].elemt.options.length; i++) {
					if (DataBindding.elemts[("#" + obj.elemt.id)].elemt.options[i].value == DataBindding.elemts[("#" + obj.elemt.id)]
						.value) {
						DataBindding.elemts[("#" + obj.elemt.id)].elemt.selectedIndex = i;
						DataBindding.elemts[("#" + obj.elemt.id)].index = i
						DataBindding.elemts[("#" + obj.elemt.id)].optValue = DataBindding.elemts[("#" + obj.elemt.id)].elemt.options[i]
							.innerHTML;
						break;
					}
				}
	...
	/**
	 * 如果通过js创建了html并且使用了databinding，
	 * 要调用refreshElemts来重新扫描到databinding中
	 */
		...
		for (var i in tags) {
			if (tags[i].nodeType == 1) {
				if (tags[i].getAttribute("class") != null &&
					__hasArrItem(tags[i].getAttribute("class").split(" "), clazname)) { //如果某元素的class值为所需要
		...
		if (id == "") {
			...
			for (var p in DataBindding.replaceElements) {
				...
				for (var pa in ar) {
					var arr = (ar[pa].split("}}"));
					for (var i = 0; i < arr.length; i++) {
						if (i == 0 && arr[i].indexOf("$") > -1 && arr[i].indexOf(".") > -1) {
